...
3. con la terza formulazione più complicata perché contiamo come una sola configurazione, per via delle simmetrie, configurazioni precedentemente diverse, il numero degli stati diminuisce ulteriormente. Per sapere di quanto diminuisce... beh, di certo esisterà una formula che, come nei casi precedenti, conti elegantemente gli stati in funzione di $n$ ed $m$. Tuttavia stavolta ricavarla è molto meno immediato, quindi non ci ho badato troppo: la mia natura ingegneristica ha preso il sopravvento e, contento di aver raggiunto il caso ottimale, ho beatamente proseguito col resto del problema senza stare ad indagare troppo questo calcolo[^2]

Il punto è infatti che all'aumentare del numero di ciotole e palline, il terzo metodo permette di limitare la numerosità degli stati, mantenendo quindi il problema più agilmente risolvibile per un computer. Per esempio, con 12 ciotole e 6 palline, il primo metodo prevederebbe $12^6=2985984$ stati, il secondo ${17 \choose 6}=12376$, il terzo 561. E siccome in seguito vedremo che ci sarà da risolvere un sistema, meglio lavorare con sistemi da 561 variabili rispetto che 2985984!. La formulazione del secondo metodo è comunque utile a livello umano per raccontare cosa succede, quindi la riprenderò comunque, a volte, nel discorso.

[^1]: Per la cronaca, è il "classico" problema delle stelline e sbarrette.
[^2]: Dopo maggiori ricerche, il numero degli stati derivati da $n$ ciotole ed $m$ palline secondo la terza formulazione segue [questa sequenza](https://oeis.org/A052307), che descrive il numero di braccialetti, o collane, che possono essere composte con $n$ perline delle quali $m$ sono nere ed $n - m$ sono bianche. La definizione matematica è un po' contorta: $$T(n,m) = \frac{1}{2} \binom{\lfloor n/2 \rfloor - (m \bmod 2)(1 - (n \bmod 2))}{\lfloor m/2 \rfloor}+ \frac{1}{2n} \sum_{\substack{d \mid n \\ d \mid m}}\varphi(d) \binom{n/d}{m/d} $$dove $\phi(n) = \{ 1\leq m \leq n \; : \;  \gcd(m,n)=1\}$ è la [funzione di Eulero, o toziente](https://it.wikipedia.org/wiki/Funzione_%CF%86_di_Eulero), che conta i numeri interi compresi tra $1$ ed $n$ che sono coprimi con $n$, ovvero che non hanno nessun fattore primo in comune con $n$.
...
Le funzioni qui di seguito si occupano di generare le variabili finora introdotte. Di ogni funzione propongo anche alcuni esempietti di esecuzione, con l'opzione `verbose` attiva, in modo da facilitarne la comprensione. Possiamo inoltre usare il tipo `Rational` per questi calcoli in modo da ottenere una soluzione nello stesso formato proposto dal testo.
...
$$ \begin{cases} k_i^R = 0 & i\in R\\ k_i^R = 1+\displaystyle\sum_{j\notin R}p_{ij}k_j^R & i\notin R \end{cases} $$

Trascurando, in generale, gli stati ricorrenti (nel nostro caso, di nuovo, sarebbe trascurare un solo stato) che non danno contributo, essendo che per loro $k_i^R=0$, possiamo considerare la sola seconda equazione e vederla in forma vettoriale ${\bf{k}} = {\bf{1}} + Q\bf{k}$, da cui si ricava il sistema $(I-Q)\bf{k} = \bf{1}$, che possiamo risolvere in julia in modo classico con l'operatore backslash `\ ` o usando metodi più sofisticati dal pacchetto `IterativeSolvers`.

````julia:ex14
# https://en.wikipedia.org/wiki/Absorbing_Markov_chain
Q = P[2:end,2:end]
if size(Q)[1] > 1 # c'è davvero un sistema da risolvere
	k = (I(nstates-1)-Q)\ones(Rational,nstates-1) # metodo classico
...
che corrisponde precisamente alla soluzione esatta, che numericamente sarebbe:
...
````julia:ex19
6277/12 # la soluzione esatta, come da testo, per G(4,5)
````

Di seguito i grafici relativi ai vari casi analizzati in G(4,5):

- 2 ciotole, 2 palline
\fig{/assets/literate_outputs/project_euler/2bowls_2balls_graph.svg}
- 2 ciotole, 3 palline
\fig{/assets/literate_outputs/project_euler/2bowls_3balls_graph.svg}
- 2 ciotole, 4 palline
\fig{/assets/literate_outputs/project_euler/2bowls_4balls_graph.svg}
- 2 ciotole, 5 palline
\fig{/assets/literate_outputs/project_euler/2bowls_5balls_graph.svg}
- 3 ciotole, 2 palline
\fig{/assets/literate_outputs/project_euler/3bowls_2balls_graph.svg}
- 3 ciotole, 3 palline
\fig{/assets/literate_outputs/project_euler/3bowls_3balls_graph.svg}
- 3 ciotole, 4 palline
\fig{/assets/literate_outputs/project_euler/3bowls_4balls_graph.svg}
- 3 ciotole, 5 palline
\fig{/assets/literate_outputs/project_euler/3bowls_5balls_graph.svg}
- 4 ciotole, 2 palline
\fig{/assets/literate_outputs/project_euler/4bowls_2balls_graph.svg}
- 4 ciotole, 3 palline
\fig{/assets/literate_outputs/project_euler/4bowls_3balls_graph.svg}
- 4 ciotole, 4 palline
\fig{/assets/literate_outputs/project_euler/4bowls_4balls_graph.svg}
- 4 ciotole, 5 palline
\fig{/assets/literate_outputs/project_euler/4bowls_5balls_graph.svg}
