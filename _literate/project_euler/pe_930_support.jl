function F(::Type{T}, nbowls, nballs; verbose=false, plot_graph=false) where {T<:Number}
	...
	if verbose
		...
		## with a real plot:
		spy(P)
		savefig(joinpath("_assets","literate_outputs", "$(nbowls)bowls_$(nballs)balls_spy.svg")); # hide
	end
	if plot_graph
		...
		method=:circular,
		axis_buffer=0.33)
		savefig(joinpath("_assets","literate_outputs", "project_euler", "$(nbowls)bowls_$(nballs)balls_graph.svg")); # hide
	end
	Q = P[2:end,2:end]
...
function G(N,M)
	...
	for n in 2:N, m in 2:M
		@time out = F(Rational{Int},n,m, plot_graph=true)
